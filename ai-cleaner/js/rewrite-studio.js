...
let state={draft:'',locks:[],variant:0,composing:false,lastSource:'',generating:false};
...
async function generate(variantBump=false){
  if(state.generating)return;const text=sourceText();if(!text.trim())return app.showToast('먼저 원본 글을 입력해 주세요.');
  ...
  try{
    ...
    setTimeout(()=>{$('#rewriteProgress').hidden=true;},900);app.showToast(variantBump?'다른 초안을 만들었습니다.':'새 초안을 만들었습니다.');resetDirect();
...
function compareTyped(){
  const target=Array.from(targetText()),typed=Array.from($('#directTyped').value),compareN=state.composing?Math.max(0,typed.length-1):typed.length;let correct=0,first=-1;
...
function resetDirect(){state.composing=false;$('#directTyped').value='';compareTyped();}
...
$('#directTarget').addEventListener('change',resetDirect);$('#directHideTarget').addEventListener('change',compareTyped);$('#directReset').addEventListener('click',resetDirect);
$('#directTyped').addEventListener('compositionstart',()=>{state.composing=true;compareTyped();});$('#directTyped').addEventListener('compositionend',()=>{state.composing=false;compareTyped();});$('#directTyped').addEventListener('input',compareTyped);
