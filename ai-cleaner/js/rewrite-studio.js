...
function switchTab(name){
  name=name==='verify'?'verify':'draft';state.activeTab=name;$$('[data-rewrite-tab]').forEach(b=>b.classList.toggle('active',b.dataset.rewriteTab===name));$('#rewriteDraftPane').hidden=name!=='draft';$('#rewriteVerifyPane').hidden=name!=='verify';
  if(name==='verify'){
    $('#directTarget').value='original';const current=targetText();if(state.directTargetText&&state.directTargetText!==current)resetDirect();else{state.directTargetText=current;state.directTargetChars=Array.from(current);queueDirectCompare();}setTimeout(()=>$('#directTyped').focus(),0);
  }
}
...
function open(){if(!sessionRestored){restoreSession();sessionRestored=true;}refreshForSource();switchTab(state.activeTab||'draft');renderValidation();app.configureEditors($('#rewritePanel'));}
