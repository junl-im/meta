...
ns.createPanelManager=function createPanelManager({ids=[],breakpoint=980,anchor=()=>null,offsets={},storagePrefix='ai-cleaner-panel-',legacyPrefix='v66-pos-',topMin=76}={}){
  let z=220,lastMobile=innerWidth<=breakpoint;
  ...
  function positionDefault(panel){
    if(!panel||innerWidth<=breakpoint)return false;const a=anchor();if(!a)return false;
    ...
    panel.style.left=left+'px';panel.style.top=top+'px';panel.style.right='auto';panel.style.bottom='auto';return true;
  }
  function clamp(panel){
    if(!panel||panel.hidden||innerWidth<=breakpoint)return;
    const r=panel.getBoundingClientRect(),maxW=Math.max(360,innerWidth-16),maxH=Math.max(260,innerHeight-topMin-8);
    if(r.width>maxW)panel.style.width=maxW+'px';if(r.height>maxH)panel.style.height=maxH+'px';
    const rr=panel.getBoundingClientRect(),left=Math.max(8,Math.min(innerWidth-rr.width-8,rr.left)),maxTop=Math.max(topMin,innerHeight-rr.height-8),top=Math.max(topMin,Math.min(maxTop,rr.top));
  ...
  function closeTop(){
    ...
    if(!visible[0])return null;visible[0].hidden=true;setMobileExpanded(visible[0],false);return visible[0];
  }
  function makeDraggable(panel){
    ...
    handle.addEventListener('pointermove',e=>{if(!drag||drag.id!==e.pointerId)return;const maxX=Math.max(8,innerWidth-panel.offsetWidth-8),maxY=Math.max(topMin,innerHeight-panel.offsetHeight-8);panel.style.left=Math.max(8,Math.min(maxX,e.clientX-drag.dx))+'px';panel.style.top=Math.max(topMin,Math.min(maxY,e.clientY-drag.dy))+'px';});
  ...
  function handleResize(){
    const mobile=innerWidth<=breakpoint,crossed=mobile!==lastMobile;lastMobile=mobile;
    for(const id of ids){const panel=get(id);if(!panel)continue;if(crossed)setMobileExpanded(panel,false);if(!mobile&&panel.dataset.defaultPosition==='pending'&&!panel.hidden&&positionDefault(panel))panel.dataset.defaultPosition='done';clamp(panel);}
  }
