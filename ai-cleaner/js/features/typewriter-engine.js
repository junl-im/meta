...
ns.createTypewriterEngine=function createTypewriterEngine({split=Array.from,raf=requestAnimationFrame,caf=cancelAnimationFrame}={}){
  ...
  function schedule(){if(state.running&&!state.paused&&!state.frame)state.frame=raf(step);}
  function step(ts){
    state.frame=0;if(!state.running||state.paused)return;
  ...
  function stop(){const wasRunning=state.running;state.running=false;state.paused=false;if(state.frame)caf(state.frame);state.frame=0;hooks.onStop?.(snapshot());return wasRunning;}
  function togglePause(){
    if(!state.running)return state.paused;
    state.paused=!state.paused;
    if(state.paused){if(state.frame)caf(state.frame);state.frame=0;}else{state.last=0;schedule();}
    hooks.onPause?.(state.paused,snapshot());return state.paused;
  }
