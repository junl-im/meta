...
ns.createAnalysisCoordinator=function createAnalysisCoordinator({executor,syncExecutor=executor,onCancel=()=>{},setTimer=setTimeout,clearTimer=clearTimeout,idle=typeof requestIdleCallback==='function'?requestIdleCallback:null,cancelIdle=typeof cancelIdleCallback==='function'?cancelIdleCallback:null,now=()=>performance.now()}={}){
  ...
  function clearHandles(){
    ...
    if(idleId!==null){if(cancelIdle){try{cancelIdle(idleId);}catch(_){}}idleId=null;}
  }
  function cancel(){sequence++;pending=false;clearHandles();try{onCancel();}catch(_){}return sequence;}
  async function execute(token,text,options,onResult,onError){
    idleId=null;if(token!==sequence)return false;const started=now();
    try{
      const result=await executor(text,options);if(token!==sequence)return false;
      pending=false;onResult?.(result,{token,durationMs:Math.max(0,now()-started)});return true;
  ...
  function schedule(text,options={},config={}){
    ...
    timerId=setTimer(()=>{
      timerId=null;if(token!==sequence)return;
      const run=()=>{if(token!==sequence)return;void execute(token,text,options,config.onResult,config.onError);};
      if(idle){
        try{idleId=idle(run,{timeout:Math.max(80,Number(config.idleTimeout)||500)});}
        catch(_){idleId=null;run();}
      }else run();
    },delay);
    return token;
  }
