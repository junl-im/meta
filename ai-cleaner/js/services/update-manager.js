...
ns.createUpdateManager=function createUpdateManager({currentVersion='local',draftKey='ai-cleaner-update-draft-v1',reloadKey='ai-cleaner-refresh-target',fetchImpl,storage,locationObj,isBlocked=()=>false,captureDraft=()=>null,restoreDraft=()=>{},now=()=>Date.now(),AbortControllerCtor=typeof AbortController!=='undefined'?AbortController:null,requestTimeoutMs=8000,setRequestTimer=(fn,ms)=>setTimeout(fn,ms),clearRequestTimer=id=>clearTimeout(id)}={}){
  ...
  async function check(){
    ...
    const run=++runSeq,controller=AbortControllerCtor?new AbortControllerCtor():null;activeController=controller;busy=true;let armedLatest='',requestTimer=0;
    try{
      const fetchOptions={cache:'no-store'};if(controller?.signal)fetchOptions.signal=controller.signal;
      if(controller&&requestTimeoutMs>0)requestTimer=setRequestTimer(()=>controller.abort(),requestTimeoutMs);
    ...
    finally{if(requestTimer)clearRequestTimer(requestTimer);if(activeController===controller)activeController=null;if(run===runSeq)busy=false;}
  }
