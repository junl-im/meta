...
const context={
  ...
  setTimeout,clearTimeout,setInterval,clearInterval,Date,Map,Set,JSON,String,Array,Math,Number,Object,Error,Promise,TextDecoder,ArrayBuffer,Uint8Array
};
...
{
  const importer=M.createFileImportService({maxBytes:32});assert(importer.rtfToText('{\\rtf1\\ansi 테스트\\par 다음}').includes('테스트\n다음'),'RTF import failed');assert(importer.looksBinary('abc\u0000def'),'binary text detection failed');assert(importer.parse('a.txt','plain')==='plain','plain import failed');let tooLarge=false;try{await importer.read({name:'a.txt',size:33,arrayBuffer:async()=>new Uint8Array(33).buffer});}catch(e){tooLarge=e.code==='FILE_TOO_LARGE';}assert(tooLarge,'file size guard failed');const utf8Bytes=new TextEncoder().encode('hello');const ok=await importer.read({name:'a.txt',size:utf8Bytes.length,arrayBuffer:async()=>utf8Bytes.buffer});assert(ok.text==='hello'&&ok.encoding==='utf-8','UTF-8 file read failed');const cp949=new Uint8Array([191,192,183,161,181,200,32,199,209,177,219,32,198,196,192,207]);const legacy=await importer.read({name:'old-korean.txt',size:cp949.length,arrayBuffer:async()=>cp949.buffer});assert(legacy.text==='오래된 한글 파일'&&legacy.encoding==='euc-kr','CP949/EUC-KR fallback decode failed');const rtf949=String.raw`{\rtf1\ansi\ansicpg949 \'c5\'d7\'bd\'ba\'c6\'ae\par \'b4\'d9\'c0\'bd}`;assert(importer.rtfToText(rtf949).includes('테스트\n다음'),'CP949 RTF hex decode failed');
}
{
  ...
  class TimeoutAbortController{constructor(){const listeners=[];this.signal={aborted:false,addEventListener:(type,fn)=>{if(type==='abort')listeners.push(fn);}};this.listeners=listeners;TimeoutAbortController.last=this;}abort(){if(this.signal.aborted)return;this.signal.aborted=true;for(const fn of this.listeners)fn();}}
  const requestTimers=new Map();let requestTimerId=0,clearedRequestTimer=0;const timeoutUpdate=M.createUpdateManager({currentVersion:'1.3.1',AbortControllerCtor:TimeoutAbortController,requestTimeoutMs:25,setRequestTimer:(fn,ms)=>{assert(ms===25,'update request timeout duration failed');const id=++requestTimerId;requestTimers.set(id,fn);return id;},clearRequestTimer:id=>{clearedRequestTimer=id;requestTimers.delete(id);},fetchImpl:(_url,{signal})=>new Promise((resolve,reject)=>{signal.addEventListener('abort',()=>reject(new Error('aborted')));}),now:()=>100});const timeoutCheck=timeoutUpdate.check();assert(timeoutUpdate.busy&&requestTimers.size===1,'update request timeout should be armed');[...requestTimers.values()][0]();assert(await timeoutCheck===false&&!timeoutUpdate.busy&&TimeoutAbortController.last.signal.aborted&&clearedRequestTimer===1,'timed-out update check must abort and release busy state');
...

{
  ...
  fields.osTask.value='육아 · 아이와 갈 곳';os.selectFactoryMode('daily_topics',{remember:false});os.setAutoDaily(true);const todayPack=os.buildTaskPackSync();assert(todayPack.automation.autoDaily===true&&todayPack.automation.dailyAngle&&todayPack.factory.mode==='daily_topics'&&/^\d{4}-\d{2}-\d{2}$/.test(todayPack.dailyKey),'daily local automation metadata must be present');
  fields.osDisplayName.value='가'.repeat(100);fields.osPreferences.value=Array.from({length:60},(_,i)=>`키${i}: ${'값'.repeat(1200)}`).join('\n');const boundedProfile=os.buildTaskPackSync().userProfile;assert(boundedProfile.displayName.length===80&&Object.keys(boundedProfile.preferences).length<=50&&Object.keys(boundedProfile.preferences).length>0&&Object.values(boundedProfile.preferences).every(v=>v.length<=1000),'Blog Factory saved profile bounds failed');fields.osDisplayName.value='테스트 사용자';fields.osPreferences.value='문체: 자연스럽게';
}
...
console.log('PASS Blog Factory GitHub Actions Daily Engine + local prompt builder unit checks');
